...
import study.repository.SignatureImageRepository;
import study.repository.SignatureTemplateRepository;
import study.repository.UserRepository;
import study.security.AuthenticatorTOTPService;
import study.security.SecurityUtils;
import study.service.dto.InfoSignatureDTO;
import study.service.impl.parser.InfoSignatureParser;
import study.service.mapper.CertificateMapper;
import study.service.parser.SignatureTemplateParseService;
import study.service.parser.SignatureTemplateParserFactory;
import study.utils.*;
import study.utils.AccountUtils;
import study.utils.CertificateEncryptionHelper;
import study.utils.FileIOHelper;
import study.utils.ParserUtils;
import study.web.rest.CertificateResource;
import study.web.rest.vm.request.CreateCertRsRequest;
...
public class CertificateService {
    ...
    public CertificateService(
        ...
        SystemConfigCachingService systemConfigCachingService,
    ...
    public List<Certificate> getByOwnerId(String ownerId) throws ApplicationException {
        ...
        if (!userEntityOptional.isPresent()) throw new ApplicationException(String.format("User is not exist - ownerId %s", ownerId));
    ...
    public CertificateDTO getBySerial(String serial) throws CertificateNotFoundAppException {
        ...
        if (certificateDTO == null) throw new CertificateNotFoundAppException();
    ...
    @Transactional(readOnly = true)
    public Page<Certificate> findByFilter(
        Pageable pageable,
        String alias,
        String ownerId,
        String serial,
        ...
        if (userEntityOptional.isPresent()) {
            ...
            boolean isAdmin = userAuthority
                .stream()
                .anyMatch(ua -> "ROLE_ADMIN".equals(ua.getName()) || "ROLE_SUPER_ADMIN".equals(ua.getName()));
            if (!isAdmin) {
    ...
    public String getSignatureImage(String serial, String pin) throws ApplicationException {
        ...
        if (!certificateOptional.isPresent()) throw new ApplicationException("Certificate is not found");
        ...
        Optional<SignatureTemplate> signatureTemplateOptional = signatureTemplateRepository.findFirstByUserIdOrderByCreatedDateDesc(
        ...
        if (signImageId != null) {
            ...
            if (signatureImage.isPresent()) signatureImageData = signatureImage.get().getImgData();
        }
        X509Certificate x509Certificate = cryptoTokenProxy.getX509Certificate();
        String subjectDN = x509Certificate.getSubjectDN().getName();

        SignatureTemplateParseService signatureTemplateParseService = signatureTemplateParserFactory.resolve(
            signatureTemplate.getCoreParser()
        );
    ...
    public String getSignatureImage(String serial, String pin, Object data) throws ApplicationException {
        ...
        cryptoTokenProxy.getCryptoToken().checkInitialized();
        Optional<UserEntity> userEntity = userRepository.findOneWithAuthoritiesByLogin(AccountUtils.getLoggedAccount());
        Optional<SignatureTemplate> signatureTemplateOptional = signatureTemplateRepository.findOneByUserIdAndActivated(
            userEntity.get().getId(),
            true
        );
        if (!signatureTemplateOptional.isPresent()) {
    ...
    public String getSignatureImageByTemplateId(String serial, String pin, Long templateId) throws ApplicationException, IOException {
        ...
        if (templateId == DEFAULT_OPTION | templateId == null) {
            try (InputStream inputFileStream = fileResourceService.getTemplateFile("/templates/signature/signatureTemplate_2.0.html")) {
                ...
                SignatureTemplateParseService signatureTemplateParseService = signatureTemplateParserFactory.resolve(
                    SignatureTemplateParserType.DEFAULT
                );
    ...
    public String getBase64OTPQRCode(String serial, String pin) throws ApplicationException {
        Optional<Certificate> certificateOptional = certificateRepository.findOneBySerialAndActiveStatus(serial, Certificate.ACTIVATED);
        if (!certificateOptional.isPresent()) throw new ApplicationException(
            String.format("Certificate is not exist - serial: %s", serial)
        );
        CertificateDTO certificateDTO = mapper.map(certificateOptional.get());
        CryptoTokenProxy cryptoTokenProxy = cryptoTokenProxyFactory.resolveCryptoTokenProxy(certificateDTO, pin);
        cryptoTokenProxy.getCryptoToken().checkInitialized();
    ...
    public void changePIN(String serial, String oldPin, String newPin, String otp) throws ApplicationException {
        try {
            ...
            if (CertificateDTO.PKCS_11.equals(certificateDTO.getTokenType())) {
                ...
                if (dbConfiguration.getSaveTokenPassword()) certificateDTO.setEncryptedPin(symmetricService.encrypt(newPin));
            }
    ...
    public byte[] getCertReport() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (cert.getType() == 0) row.createCell(8).setCellValue("EASYSIGN"); else row.createCell(8).setCellValue("REMOTE SIGNING");
