...
public class PostRequester {
    ...
    public String post(Object data) throws IOException, RAUnAuthorized {
        ...
        Request request = Request.Post(url).useExpectContinue().bodyString(body, ContentType.APPLICATION_JSON);
        if (idToken != null) request.addHeader("Authorization", idToken);
        Response response = request.execute();
        try {
            return response.returnContent().asString(StandardCharsets.UTF_8);
        } catch (HttpResponseException ex) {
            if (ex.getStatusCode() == 401) throw new RAUnAuthorized(); else throw ex;
        }
    }
}
