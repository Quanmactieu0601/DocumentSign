package vn.easyca.signserver.core.signer.pdfsigner;

import org.apache.commons.io.IOUtils;
import vn.easyca.signserver.sign.core.sign.integrated.pdf.PartyMode;
import vn.easyca.signserver.sign.core.sign.integrated.pdf.SignPDFDto;
import vn.easyca.signserver.sign.core.sign.integrated.pdf.SignPDFPlugin;
import vn.easyca.signserver.sign.core.sign.utils.UniqueID;
...
public class PDFSigner {
    ...
    public byte[] signPDF(SigningRequest<PDFSigningData> request) throws Exception {
        initTemDir();
        String temFilePath = cacheDir + UniqueID.generate() + ".pdf";
        File file = new File(temFilePath);
        PDFSigningData content = request.getData();
        if (!file.exists())
            file.createNewFile();
        try {
