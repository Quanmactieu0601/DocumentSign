...
public class SignUtils {
    ...
    public byte[] signHash(byte[] data, String providerName, PrivateKey key, String hashAlg) throws Exception {
        ...
        if (hash == null) throw new Exception("Could not hash data");
        Cipher cipher = null;
        if (providerName == null || providerName.isEmpty()) cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding"); else cipher =
            Cipher.getInstance("RSA/ECB/PKCS1Padding", providerName);
