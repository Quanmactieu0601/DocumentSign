...
import core.services.P12ImportService;
...
public class P12CryptoToken implements CryptoToken {
    ...
    public void initPkcs12(String p12Base64, String pin) throws InitCryptoTokenException {
        if (p12Base64 == null) throw new InitCryptoTokenException("Config is null");
        modulePin = pin;
        byte[] fileContent = Base64.getDecoder().decode(p12Base64);
        InputStream is = new ByteArrayInputStream(fileContent);
        if (modulePin == null || modulePin.isEmpty()) throw new InitCryptoTokenException("Module pin is required");
        if (is == null) throw new InitCryptoTokenException("P12 input stream is required");
