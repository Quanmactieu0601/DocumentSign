package pki.cryptotoken.impl;

import core.exception.ApplicationException;
import core.exception.PinIncorrectException;
...
public class P11CryptoToken implements CryptoToken {
    ...
    public void initPkcs11() throws InitCryptoTokenException {
        if (ks == null) {
            ...
            if (pkcs11Config.isEmpty()) throw new InitCryptoTokenException("Config is empty");
            if (modulePin.isEmpty()) throw new InitCryptoTokenException("modulePin is required");
    ...
    public PrivateKey getPrivateKey(String alias) throws CryptoTokenException {
        if (this.ks == null) throw new CryptoTokenException("cryptoToken is not initialized");
        try {
            if (!ks.containsAlias(alias)) throw new CryptoTokenException("Private key alias not found");
        } catch (KeyStoreException e) {
    ...
    public PublicKey getPublicKey(String alias) throws CryptoTokenException {
        if (this.ks == null) throw new CryptoTokenException("cryptoToken is not initialized");
        try {
            if (!ks.containsAlias(alias)) throw new CryptoTokenException("Public key alias not found");
        } catch (KeyStoreException e) {
    ...
    private X509Certificate genSelfSignedCert(String alias, KeyPair keyPair, String providerName) throws Exception {
        ...
        ContentSigner signer = new BufferingContentSigner(
            new JcaContentSignerBuilder("SHA256WithRSA").setProvider(providerName).build(keyPair.getPrivate()),
            20480
        );
    ...
    public Signature getSignatureInstance(String algorithm) throws ApplicationException {
        if (StringUtils.isNullOrEmpty(algorithm)) throw new ApplicationException(-1, "Hash algorithm cannot be null or empty");

        algorithm = algorithm.trim().toUpperCase().replace("-", "");
        if (!Constants.HASH_ALGORITHMS.contains(algorithm)) throw new ApplicationException(-1, "Hash algorithm not correct");
        try {
    ...
    public void checkInitialized() throws ApplicationException {
        try {
            if (ks == null) throw new ApplicationException("Cannot init token"); else ks.aliases();
        } catch (Exception ex) {
