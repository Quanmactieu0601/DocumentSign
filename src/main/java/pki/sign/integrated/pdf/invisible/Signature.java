...
public class Signature implements SignatureInterface {
    ...
    Signature(PrivateKey privateKey, Certificate[] certificateChain, String algorithm, String provider)
        throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, IOException, CertificateNotYetValidException, CertificateExpiredException {
    ...
    public byte[] sign(InputStream content) throws IOException {
        try {
            ...
            if (StringUtils.isNotBlank(provider)) contentSignerBuilder.setProvider(provider);
            ContentSigner sha1Signer = contentSignerBuilder.build(this.privateKey);
            gen.addSignerInfoGenerator(
                new JcaSignerInfoGeneratorBuilder(new JcaDigestCalculatorProviderBuilder().build()).build(sha1Signer, cert)
            );
