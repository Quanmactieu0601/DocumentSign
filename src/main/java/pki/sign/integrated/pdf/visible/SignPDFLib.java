...
public class SignPDFLib {
    ...
    private void addSignatureImg(SignPDFDto signDTO, String tempFile) throws Exception {
        ...
        Rectangle rectangle = new Rectangle(
            signDTO.getVisibleX(),
            signDTO.getVisibleY(),
            signDTO.getVisibleX() + signDTO.getVisibleWidth(),
            signDTO.getVisibleY() + signDTO.getVisibleHeight()
        );
    ...
    private void emptySignature(SignPDFDto signDTO, String tempFile) throws Exception {
        ...
        if (this.partyMode == PartyMode.CRM_CONTRACT) {
            layer2Text =
                ...
                ": " +
                DatetimeUtils.convertDateToString(signDTO.getSignDate(), "DD/MM/YYYY") +
                addtional;
        }
        ...
        ct.setSimpleColumn(
            new Phrase(layer2Text, regularFont),
            dataRec.getLeft(),
            dataRec.getBottom(),
            dataRec.getRight(),
            dataRec.getTop(),
    ...
    private List<byte[]> preSign(String src, String fieldName, Certificate[] chain, Date signDate) throws Exception {
    ...
    private void validDTO(SignPDFDto dto) throws Exception {
        if (
            ...
            (dto.getVisibleX() < 0) ||
            (dto.getVisibleY() < 0) ||
            (dto.getVisibleWidth() < 0) ||
            (dto.getVisibleHeight() < 0)
        ) throw new Exception("Dữ liệu định dạng khung ký sai.");
    }
}
