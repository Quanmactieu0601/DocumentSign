...
public class DigestCreator {
    ...
    public byte[] hash(byte[] data, String hashAlgo) throws Exception {
        if (hashAlgo == null || hashAlgo.isEmpty() || hashAlgo.equalsIgnoreCase("sha1")) return sha1(data); else if (
            hashAlgo.equalsIgnoreCase("sha256")
        ) return sha256(data); else if (hashAlgo.equalsIgnoreCase("sha512")) return sha512(data);
    ...
    public byte[] digestWithSHAInfo(String hashAlgo, byte[] digest) throws Exception {
        ...
        if (hashAlgo == null || hashAlgo.isEmpty()) hashAlgo = Constants.HASH_ALGORITHM.SHA1;
        if (hashAlgo.equalsIgnoreCase(Constants.HASH_ALGORITHM.SHA1)) sha1oid_ =
            new ASN1ObjectIdentifier(DigestAlgorithm.SHA1.getOid()); else if (
            hashAlgo.equalsIgnoreCase(Constants.HASH_ALGORITHM.SHA256)
        ) sha1oid_ = new ASN1ObjectIdentifier(DigestAlgorithm.SHA256.getOid()); else if (
            hashAlgo.equalsIgnoreCase(Constants.HASH_ALGORITHM.SHA512)
        ) sha1oid_ = new ASN1ObjectIdentifier(DigestAlgorithm.SHA512.getOid());
        if (sha1oid_ == null) throw new Exception("Not support hash Algorithm");
        AlgorithmIdentifier sha1aid_ = new AlgorithmIdentifier(sha1oid_, null);
        DigestInfo di = new DigestInfo(sha1aid_, digest);
        return di.getEncoded();
    }
