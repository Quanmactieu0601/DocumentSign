...
public class SignatureVerificationService {
    ...
    public SignatureVerificationResponse verifyRaw(SignatureVerificationRequest request) throws Exception {
        if (request.getHashAlgorithm() == null) request.setHashAlgorithm(Constants.HASH_ALGORITHM.SHA1);
        SignatureVerificationResponse response = new SignatureVerificationResponse();
        CertificateDTO certificateDTO = certificateService.getBySerial(request.getSerial());
        if (certificateDTO == null) throw new CertificateNotFoundAppException();
        response.setCertificate(certificateDTO.getRawData());
        ...
        OCSP.RevocationStatus revoStatus = OCSP.check(
            x509Certificate,
            (X509Certificate) ks.getCertificate("root"),
            OCSP.getResponderURI(X509CertImpl.toImpl(x509Certificate)),
            null,
            null
        );
        if (revoStatus.getCertStatus().toString().trim().equals(RevocationStatus.REVOKED.toString())) revocationStatus =
            RevocationStatus.REVOKED; else if (
            revoStatus.getCertStatus().toString().trim().equals(RevocationStatus.GOOD.toString())
        ) revocationStatus = RevocationStatus.GOOD; else if (
            revoStatus.getCertStatus().toString().trim().equals("UNKNOWN")
        ...
        SignatureValidator rawValidator = new SignatureValidator();
        for (SignatureVerificationRequest.Element element : request.getElements()) {
            boolean result = rawValidator.verify(
    ...
    private CertificateVfDTO getCertificateInfo(
        PdfPKCS7 pkcs7,
        ...
        RevocationStatus revocationStatus = RevocationStatus.UNCHECKED;
        if (isSigningCert) revocationStatus = checkRevocation(pkcs7, cert, issuerCert, signDate);
    ...
    public VerificationResponseDTO verifyPDF(InputStream pdfStream) throws ApplicationException {
        try {
            ...
            for (String name : signatureNames) {
                ...
                X509Certificate issuerCert = isEasyCACert
                    ? (X509Certificate) ks.getCertificate("root")
                    : (certs.length > 1 ? (X509Certificate) certs[1] : null);
                int signCertIndex = 0;
    ...
    private CertificateVfDTO getCertificateInfoDoc(X509Certificate cert, Date signDate)
        throws KeyStoreException, CertificateException, NoSuchAlgorithmException, IOException, ApplicationException, CertPathValidatorException {
        CertificateVfDTO certificateVfDTO = new CertificateVfDTO();
        ...
        OCSP.RevocationStatus revoStatus = OCSP.check(
            cert,
            (X509Certificate) ks.getCertificate("root"),
            OCSP.getResponderURI(X509CertImpl.toImpl(cert)),
            null,
            null
        );
        if (revoStatus.getCertStatus().toString().trim().equals(RevocationStatus.REVOKED.toString())) revocationStatus =
    ...
    private CertificateVfDTO getCertificateInfoXml(X509Certificate cert, boolean integrity)
        throws KeyStoreException, CertificateException, IOException, NoSuchAlgorithmException, ApplicationException, CertPathValidatorException {
        CertificateVfDTO certificateVfDTO = new CertificateVfDTO();
