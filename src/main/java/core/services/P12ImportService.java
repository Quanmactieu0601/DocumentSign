...
public class P12ImportService {
    ...
    @Autowired
    public P12ImportService(
        CertificateService certificateService,
        UserApplicationService userApplicationService,
        CertPackageService certPackageService,
        ...
        CertificateGenerateService certificateGenerateService,
    ...
    public CertificateDTO insert(ImportP12FileDTO input) throws ApplicationException {
        ...
        if (dbConfiguration.getSaveTokenPassword()) certificateDTO.setEncryptedPin(symmetricService.encrypt(input.getPin()));
    ...
    private String getAlias(String inputAlias, P12CryptoToken cryptoToken) throws CryptoTokenException {
        if (inputAlias != null && !inputAlias.isEmpty()) return inputAlias;
        List<String> aliases = cryptoToken.getAliases();
        if (aliases != null && aliases.size() > 0) return aliases.get(0);
            ...
            try {
                if (!ExcelUtils.checkIfRowIsEmpty(row)) {
                    ...
                    if (isP12) {
                        ...
                        String base64Certificate = formatter
                        ...
                        CertificateDTO certificateDTO = certificateGenerateService.saveAndInstallCert(
                            certValue,
                            alias,
                            ownerId,
                            cryptoToken
