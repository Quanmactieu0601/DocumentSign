...
public class PDFSigningService {
    ...
    public PDFSigningService(
        ...
        InvisiblePDFSigning invisiblePDFSigning
    ) {
    ...
    public SigningResponse invisibleSign(SigningRequest request) throws Exception {
        if (request == null) throw new BadServiceInputAppException("dont have element to sign", null);
        ...
        for (SigningRequestContent data : dataList) {
            byte[] pairResult = invisiblePDFSigning.signPdf(
                data.getData(),
                "",
                "",
                privateKey,
                new Certificate[] { x509Certificates },
                signatureAlgorithm,
                providerName
            );
