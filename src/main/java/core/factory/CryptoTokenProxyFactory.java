...
import core.domain.CertificateDTO;
import core.domain.TokenInfo;
...
import pki.cryptotoken.impl.P11CryptoToken;
import pki.cryptotoken.impl.P11ProtectServerCryptoToken;
import pki.cryptotoken.impl.P12CryptoToken;
import pki.sign.cache.AbstractCachedObject;
import pki.sign.cache.GuavaCache;
import study.config.HsmConfig;
import study.config.HsmTypeConstant;
import study.domain.Certificate;
import study.security.AuthenticatorTOTPService;
import study.service.SystemConfigCachingService;
...
public class CryptoTokenProxyFactory {
    ...
    public CryptoTokenProxyFactory(
        SymmetricEncryptors symmetricService,
        P12CryptoToken p12CryptoToken,
        P11ProtectServerCryptoToken p11ProtectServerCryptoToken,
        P11CryptoToken p11CryptoToken,
        HsmConfig hsmConfig,
    ...
    public CryptoTokenProxy resolveCryptoTokenProxy(CertificateDTO certificateDTO, String pin) throws ApplicationException {
        if (certificateDTO == null) throw new ApplicationException("Certificate is not null");
    ...
    public CryptoTokenProxy resolveCryptoTokenProxy(CertificateDTO certificateDTO, String pin, String otp) throws ApplicationException {
        ...
        if (useOTP) {
            if (StringUtils.isEmpty(certificateDTO.getSecretKey())) throw new ApplicationException(
                "OTP is not configured, please contact to admin to get secret key"
            );
            if (StringUtils.isEmpty(otp)) throw new ApplicationException("OTP is empty");
    ...
    private CryptoTokenProxy getCryptoTokenProxy(CertificateDTO certificateDTO, String pin) throws ApplicationException {
        ...
        if (Certificate.PKCS_12.equals(tokenType)) {
            if (skipCheckPassword) {
                if (StringUtils.isEmpty(encryptedPinFromDB)) throw new ApplicationException("P12 DB password is empty");
                rawPin = symmetricService.decrypt(encryptedPinFromDB);
            } else {
                rawPin = pin;
            }
        } else if (Certificate.PKCS_11.equals(tokenType)) {
            if (StringUtils.isEmpty(encryptedPinFromDB)) throw new ApplicationException("HSM DB password is empty");
            rawPin = symmetricService.decrypt(encryptedPinFromDB);
            if (!rawPin.equals(pin)) throw new ApplicationException("Certificate pin not correct!");
        }
