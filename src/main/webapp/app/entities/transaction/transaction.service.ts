...
export class TransactionService {
  ...
  protected convertDateFromClient(transaction: ITransaction): ITransaction {
    const copy: ITransaction = Object.assign({}, transaction, {
      triggerTime: transaction.triggerTime && transaction.triggerTime.isValid() ? transaction.triggerTime.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.triggerTime = res.body.triggerTime ? moment(res.body.triggerTime) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((transaction: ITransaction) => {
        transaction.triggerTime = transaction.triggerTime ? moment(transaction.triggerTime) : undefined;
      });
    }
    return res;
  }
